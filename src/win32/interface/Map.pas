...
begin
  ...
  try
    ...
    Image := SoAOS_DX_LoadBMP( InterfacePath + 'MapMaskedShadowLayer.bmp', cBlackBackground, DlgWidth, DlgHeight );
    try
      if assigned( Image ) then
      begin
        DrawSub( lpDDSBack, ApplyOffset( Rect( 0, 0, DlgWidth, DlgHeight ) ), Rect( 0, 0, DlgWidth, DlgHeight ), Image, True, 170 );
      end;
    finally
      Image := nil;
    end;

    Image := SoAOS_DX_LoadBMP( InterfaceLanguagePath + 'MapColorLayer.bmp', cInvisColor );
    try
      if assigned( Image ) then
      begin
...
begin
  ...
  //if InBound then Close;
    if ptInRect( ApplyOffset( Rect( 560, 378, 560 + 61, 378 + 45 ) ), point( x, y ) ) then
    begin
      DXBack := nil;
      DXDirty := nil;
      Close;
  ...
  try
    ...
    begin
      ...
      begin
        ...
        begin
          pr := Rect( 0, 0, 61, 45 );
          lpDDSBack.bltFast( 560 + Offset.X, 378 + Offset.Y, DXBack, @pr, DDBLTFAST_WAIT );
        end;
        lpDDSFront_Flip( nil, DDFLIP_WAIT );
        MouseCursor.PlotDirty := false;
        MouseOverBack := true;
    ...
    begin
      ...
      begin
       //clean up
        ...
        begin
          pr := Rect( 0, 0, 61, 45 );
          lpDDSBack.bltFast( 560 + Offset.X, 378 + Offset.Y, DXDirty, @pr, DDBLTFAST_WAIT );
        end;
        lpDDSFront_Flip( nil, DDFLIP_WAIT );
        MouseCursor.PlotDirty := false;
        MouseOverBack := false;
